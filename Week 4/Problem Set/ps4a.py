...
def getWordScore(word, n):
    ...
    times1= len(word)
    for x in word:
        score1 += SCRABBLE_LETTER_VALUES[x]
    score1 *=times1
    if times1 == n:
        score1 +=50
    return score1
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while sum(hand.values()) > 0:
        ...
        if word == '.':
            ...
            print('Goodbye! Total score: ', score_final, 'points.')
            break
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(str(word), 'earned', getWordScore(word, n), 'points.')
                score_final +=getWordScore(word, n)
                # Update the hand 
