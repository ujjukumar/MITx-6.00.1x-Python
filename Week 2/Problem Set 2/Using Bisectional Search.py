...
mp_low = round(balance/12, 2)
mp_high = round(balance*((1+ mon_int_rate)**12)/12, 2)
while balance > 0:
    min_bal = round((mp_low + mp_high)/2, 2)
    for mon in range(1,13):
        mon_unpaid = balance - min_bal
        balance = round(mon_unpaid + (mon_unpaid*mon_int_rate), 2)
        mon +=1
    if balance < 0:
        mp_high = min_bal
    elif balance > 0:
    ...
    balance =x
    ans = min_bal
    if abs(mon_unpaid) < 0.1:
        break
            
        
print('Remainder balance: ' +str(ans))